...
//		return false;
		...
		for (Coffee coffee : coffees) {
			if(coffee.getName().equals(name)) { // 메게변수로 넘어온 이름을 가진 커피 객체를 찾음!
...
//	2. 만약 sale 이 true 라면 10% 차감한 금액을 리턴.
	...
	// 메뉴 카테고리 검색
	// 빙수 매개변수로 넘어오면, 빙수 카데고리의 모든 메뉴를 리턴.
	...
	public void changeTrue(ArrayList<Coffee> searchKind) {
		...
		// 순수 자바에서는 값이 아닌 주고사 복사되면서 들어가기 때문에
		// 검색결과 객체를 바로 수정해준다 하더라
